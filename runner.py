...
class TFIDF_runner:
	...
	def __init__(self, dataset, cla):
		...
		object_token = tfidf_Tokenization('./data/{}/{}'.format(dataset, cla))

		token = object_token.token_X()
		seeds = object_token.modify_seeds()

		self.object_tfidf = tfidf('./data/{}/{}'.format(dataset, cla), token, seeds)
...
class W2V_Runner:
	...
	def __init__(self, dataset, cla):
		...
		object_token = w2v_Tokenization('./data/{}/{}'.format(dataset, cla), 100, 5, 1, 4)

		token, word_dic = object_token.get_token_wordDic()
		seeds = object_token.modify_seeds()

		self.object_w2v = Word2vector('./data/{}/{}'.format(dataset, cla), token, word_dic, seeds)
