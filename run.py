...
class W2V_Runner:
	...
	def __init__(self, dataset, cla):
		...
		object_token = w2v_Tokenization('./data/test/testdata/{}/{}'.format(dataset, cla), 100, 5, 1, 4)

		token, word_dic = object_token.get_token_wordDic()
		seeds = object_token.modify_seeds()

		self.object_w2v = Word2vector('./data/test/testdata/{}/{}'.format(dataset, cla), token, word_dic, seeds)
